...
yes I used this for my Azure theme example program
"""
...
root = tk.Tk()
root.title("Ttk widget factory")
root.option_add("*tearOff", 0) # This is always a good idea
...
input_frame.grid(row=0, column=1, padx=10, pady=(30, 10), sticky="nsew")
...
# Spinbox
spinbox = ttk.Spinbox(input_frame, from_=0, to=100, increment=0.1)
spinbox.insert(0, "Spinbox")
spinbox.grid(row=1, column=0, padx=5, pady=10, sticky="ew")

# Combobox
combobox = ttk.Combobox(input_frame, values=combo_list)
combobox.current(0)
combobox.grid(row=2, column=0, padx=5, pady=10,  sticky="ew")

# Read-only combobox
readonlycombo = ttk.Combobox(input_frame, state="readonly", values=readonly_combo_list)
readonlycombo.current(0)
readonlycombo.grid(row=3, column=0, padx=5, pady=10,  sticky="ew")

# Create a Frame for buttons
button_frame = ttk.Frame(root, padding=(0, 0, 0, 10))
button_frame.grid(row=2, column=1, padx=10, pady=(20, 10), sticky="nsew")
button_frame.columnconfigure(index=0, weight=1)

# Menu for the Menubutton
menu = tk.Menu(button_frame)
...
# Menubutton
menu_button = ttk.Menubutton(button_frame, text="Menubutton", menu=menu, direction="below")
menu_button.grid(row=0, column=0, padx=5, pady=(0, 10), sticky="nsew")

# OptionMenu
option_menu = ttk.OptionMenu(button_frame, e, *option_menu_list)
option_menu.grid(row=1, column=0, padx=5, pady=10, sticky="nsew")

# Button
button = ttk.Button(button_frame, text="Button", command=lambda: print("Choosen color: ", clr.askcolor(title="Colorchooser", color="#007fff")[1]))
...
# Treeview
treeview = ttk.Treeview(tree_frame, selectmode="extended", yscrollcommand=tree_scroll.set, columns=(1, 2), height=12)
...
# Label
label = ttk.Label(tab_1, text="Ttk widget factory", justify="center")
...
# Show messagebox question on close request
root.protocol("WM_DELETE_WINDOW", lambda: root.destroy() if msg.askyesno("Quit?", "Do you really want to quit from ttk widget factory?") is True else None)
root.mainloop()